...
if(e.target !== e.currentTarget) 
{
    ...
    {
        if(displayLength > 11) 
        { // if long equation make font smaller;
         equation.style.fontSize = "20px";
        }
       if(displayLength > 22) 
        { //clear display and make font bigger;
            alert("That is limit!");    
            clear();        
        }  
    }
    ...
    checkLength(equation.innerHTML.length); // every time when was pressed button              
     ...
     if(inputVal == "C") 
     {// invocation funct when "C" button press 
       clear();
     }
        ...
        try { //Here is all count up. RegExp for divide and multiply
         ...
         if(!(/[a-zA-Z]$/.test(result))){ //force-major situation as(infinity, undefined etc.);               
